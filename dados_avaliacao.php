...
if(!empty($_POST['token']) && !empty($_POST['course']) && !empty($_POST['assignment'])) {
    ...
    // Obtem grupos do curso e alunos em cada grupo
    $requisicao_grupos = "https://pucminas.instructure.com/api/v1/courses/$courses/groups?per_page=100";
    $arquivo_grupos = file_get_contents($requisicao_grupos, false, $context);
    $data_grupos = json_decode($arquivo_grupos);

    // monta um array com a lista dos alunos e as informações do seu grupo
    // Monta array de usuários, grupo
    $alunos_array = array();
    $group_num = 1;
    foreach ($data_grupos as $grupo) {    
        $requisicao_alunos = "https://pucminas.instructure.com/api/v1/groups/$grupo->id/memberships";
        $arquivo_alunos = file_get_contents($requisicao_alunos, false, $context);
        $data_alunos = json_decode ($arquivo_alunos);

        foreach ($data_alunos as $aluno) {
        $alunos_array[] = array("id" => $aluno->user_id, "grupo_id" => $grupo->id, "grupo_num" => $group_num, "grupo_nome" => $grupo->name);
        }
        $group_num++;
    }
    ...
    //Pegar notas e comentários das revisões já realizadas (a partir da rubrica)
    ...
    foreach ($alunos_final as $dados_aluno) {
        ...
        // inclui informações do grupo do aluno
        $aluno->grupo_id = '';
        $aluno->grupo_nome = '';
        foreach ($alunos_array as $aluno_array) {
            if ($aluno_array['id'] == $aluno->id) {
                $aluno->grupo_id = $aluno_array['grupo_id'];
                $aluno->grupo_num = $aluno_array['grupo_num'];
                $aluno->grupo_nome = $aluno_array['grupo_nome'];
                break;
            }
        }   
    ...
    function cmp($a, $b)  {  return $a->nome > $b->nome;}
    ...
    $dict_criteria = array();
    foreach ($dados_avalicoes_realizadas->data as $data_rub) {
        $dict_criteria[$data_rub->id] = $data_rub->description;
    }

    $avaliacao->comentarios = array();
    foreach ($dados_avalicoes_realizadas->assessments as $avaliacao_realizada) {
        foreach ($avaliacoes as $avaliacao) {
            if ($avaliacao_realizada->assessor_id == $avaliacao->assessor_id && $avaliacao_realizada->artifact_id == $avaliacao->asset_id) {
                ...
                //Comentarios do aluno na rubrica
                foreach ($avaliacao_realizada->data as $data) {
                    if($data->comments != "") {
                        $avaliacao->comentarios[] = $dict_criteria[$data->criterion_id]." - ".$data->comments;
                    }
                }
            }
        }
    ...
    foreach ($alunos as $aluno) {
        ...
        foreach ($avaliacoes as $avaliacao) {
            if ($avaliacao->id_aluno == $id_aluno) {
                ...
                //Adicionar o comentários feitos na Rubrica
                if($avaliacao->comentarios) {
                    $aluno->comentarios[] = array ("autor" => $avaliacao->assessor_id, "nome_autor" => $avaliacao->nome, "texto" => $avaliacao->comentarios);
                }
            }
        }
        ...
        $dados_export[] = array("id_aluno" => $id_aluno, 
                                "nome" => $nome, 
                                "comentarios" => $aluno->comentarios, 
                                "grupo_id" => $aluno->grupo_id, 
                                "grupo_num" => $aluno->grupo_num,
                                "grupo_nome" => $aluno->grupo_nome,
                                "nota_canvas" => $nota_canvsas, 
                                "nota_media" => $nota_media, 
                                "estado" => $estado, 
                                "cor" => $cor, 
                                "url_download" => $link_download, 
                                "avaliadores" => $avaliacoes_array);
